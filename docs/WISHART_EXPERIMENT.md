...
### 4.4 num_reads 영향 분석 (N=100, num_reads=1 vs 200)

num_reads=1 (단일 SA 실행)로 동일한 alpha sweep을 수행하여, SA sampling 횟수가 결과에 미치는 영향을 측정.

#### num_reads=1 결과

| Alpha | SA 성공률 | 에너지 정확도 | 평균 해밍거리 |
|-------|----------|-------------|-------------|
| 0.50  | 0%       | 93.24%      | 51.8        |
| 0.60  | 0%       | 92.41%      | 54.6        |
| 0.70  | 0%       | 89.60%      | 48.5        |
...
**전 구간 0% 성공.** num_reads=200에서 100% 풀리던 alpha=0.95~1.0도 전부 실패.

#### num_reads=200 vs num_reads=1 비교

| Alpha | 에너지 정확도 (reads=200) | 에너지 정확도 (reads=1) | 차이 |
|-------|------------------------|----------------------|------|
| 0.50  | 94.58% | 93.24% | -1.3% |
| 0.70  | 92.74% | 89.60% | -3.1% |
| 0.85  | 95.30% | 87.57% | **-7.7%** |
| 0.95  | **100%** | 83.68% | **-16.3%** |
| 1.00  | **100%** | 85.55% | **-14.5%** |

**핵심 관찰**:

1. **Hard 구간 (alpha ≤ 0.7)**: reads 증가 효과 미미 (1~3%). 200번 돌려도 SA가 구조적으로 metastable 상태를 탈출하지 못함.

2. **Easy 구간 (alpha ≥ 0.95)**: reads 증가 효과 결정적 (14~16%). 개별 SA run의 성공 확률이 낮지만, 200번 반복하면 최소 1번은 ground state에 도달. num_reads=1에서는 그 기회가 없어 전부 실패.

3. **해석**: hard 구간의 난이도는 SA 알고리즘 자체의 한계이며, sampling 횟수로 극복 불가. 반면 easy 구간은 단순히 확률적 탐색이므로 시도 횟수에 비례하여 성공률 증가.

### 4.5 Scaling 실험 (alpha=0.7)
...
### 4.6 Wishart vs Hard Mode 비교 (N=100)
...
### 5.4 Zero-Expectation과의 구조적 비교 (num_reads=1 벤치마크)

Zero-Expectation QUBO가 SA에게 trivially 쉬운 이유를 Wishart와 비교하여 분석.

#### num_reads=1 Scaling 비교

| N | ZeroExp 성공률 | Wishart(0.7) 성공률 | ZeroExp 해밍 | Wishart 해밍 |
|---|--------------|-------------------|------------|------------|
| 50 | **100%** | **0%** | 0 | 25.0 (50%) |
| 100 | **100%** | **0%** | 0 | 46.2 (46%) |
| 200 | **100%** | **0%** | 0 | 93.5 (47%) |
| 500 | **100%** | **0%** | 0 | 254.8 (51%) |

#### 에너지 랜드스케이프 구조 비교 (N=16, 전수 탐색)

| 특성 | ZeroExp | Wishart(0.7) |
|------|---------|-------------|
| **로컬 미니마 수** | **1개** (타겟만) | **224개** |
| **Frustrated pairs** | 0/120 (0%) | ~13% |
| **에너지 갭 (E1-E0)/\|E0\|** | **11.8%** | **~0%** |
| **해밍 shell 내 에너지 분산** | 작음 (신호 > 노이즈) | 큼 (노이즈 > 신호) |

#### 해밍거리별 에너지 프로파일 (N=16)

```
ZeroExp: d=0 → -109, d=1 → -72, d=2 → -37, d=3 → -4  (선형 증가, std~9)
Wishart: d=0 → -2.4, d=1 → -1.2, d=2 → -0.06, d=3 → +0.9  (약한 기울기, std~1.5)
```

**핵심 차이점**:

1. **Frustration-Free vs Frustrated**: ZeroExp는 모든 pair가 독립적으로 타겟 방향을 가리켜 frustration이 없음. Wishart는 랜덤 프로젝션 W로 J_ij를 구성하므로 pair 간 모순(frustration)이 발생하여 로컬 미니마를 다수 생성.

2. **Funnel vs Rugged**: ZeroExp의 에너지가 해밍거리에 선형 비례하여 "깔때기" 형태. SA가 어디서 출발해도 내리막길만 따라가면 타겟에 도달. Wishart는 거칠고(rugged) 로컬 미니마가 수백 개 존재하여 SA가 갇힘.

3. **에너지 갭**: ZeroExp는 ground state와 1st excited state 사이에 ~12% 상대 갭이 존재하여, 열 요동으로도 ground state에서 탈출 불가. Wishart는 갭이 ~0%로 near-degenerate 상태가 밀집.

4. **통계적 위장의 의미**: ZeroExp는 `E[q_ij]=0`을 달성하지만, **에너지 함수 구조 자체는 trivial**. 통계적 위장은 Q 행렬의 개별 계수 분포에만 적용되며, SA가 탐색하는 에너지 함수에는 영향 없음. Wishart는 랜덤 행렬 이론에 기반하여 에너지 함수 자체가 구조적으로 복잡.
