...
\begin{abstract}
QUBO 솔버를 평가하려면 정답을 미리 알면서도 풀기 어려운 인스턴스가 필요하다.
Posiform 심기는 그런 인스턴스를 유일한 정답을 갖도록 만든다. 최근 변형은 여기에
무작위 스핀글래스 항을 더하고, 심은 항을 계수 $\alpha$로 스케일해 난이도를
조절한다. 기존 연구는 이 난이도를 오직 성공률로, 즉 경험적으로만 특성화한다.
성공률은 난이도가 파라미터에 따라 달라진다는 점은 보여주지만 그 이유는 보여주지
못하고, 값이 $0\%$가 되면 인스턴스를 서로 비교할 수도 없다. 우리는 대신 실패한
시도가 정답에 얼마나 가까운지를 잰다. 실패한 경우만, 가장 어려운 인스턴스를
고정한 집단에서, 정답까지의 에너지 차와 심은 정답으로부터의 해밍 거리(HD)를
기록한다. 이 둘은 평평하던 $0\%$를 연속적인 난이도 곡선으로 풀어내고, 그 곡선이
$\Delta E = \Delta R + \alpha\Delta P$ 구조, 즉 국소최소 바닥에 커지는 심은 벌점이
더해진 형태임을 드러낸다. D-Wave Pegasus 그래프($n{=}5612$)에서 시뮬레이티드
어닐링(SA)을 돌리면 곡선은 $\alpha$에 대해 비단조다. $\Delta E$는 정점까지 올랐다가
내려가고 HD는 전 구간에서 줄며, 정점은 커지는 벌점을 실패 표본의 정답 쪽 이동이
따라잡는 지점에 생긴다. D-Wave Advantage 어닐러의 한 번의 실행도 같은 형태를
재현해, 이 구조가 SA만의 산물이 아니라 인스턴스의 성질임을 시사한다. 두 지표를 함께 보고하면 성공률이 $0$인 영역에서도
posiform-planted QUBO 벤치마크를 평가할 수 있다.
\end{abstract}
...
간격 한 칸)이다. 이 $\approx1\,\Delta_R$ 바닥은 \cite{pelofske2024hardened}의 국소최소
추측을 정량화한 것이다. 작은 $\alpha$ 극한에서 실패 표본이 $x^\star$보다 약 한 $R$
간격만 높다는 뜻이다. HD도 평탄하다($\approx73$). $\alpha$가 $10^{-3}$을 넘으면 심은
...
경쟁에서 비롯된다. 고정된 한 오답 비트열에서 $\Delta E = \Delta R + \alpha\Delta P$는
$\alpha$에 선형이다. 즉 상수인 $R$ 간격 $\Delta R$에, $\alpha$와 함께 커지는 심은
벌점 $\alpha\Delta P$가 더해진다. 그러나 실패 집단 자체는 고정이 아니다. $\alpha$가
커질수록 실패 표본은 $x^\star$ 쪽으로 이동하고(HD 감소), 그곳은 $\Delta P$가 더
작다. 따라서 관측 평균은 이 이동하는 집단을 따라가는 포락선이다. 비트열별 항
$\alpha\Delta P$가 우세한 동안은 오르고, 더 작은 $\Delta P$ 영역으로의 이동이
우세해지면 내려간다. 이 정점은 생존편향이나 선정편향의 산물이 아니다.
...
그 구조가 SA 고유의 것이 아니라 인스턴스의 성질임과 부합한다. 통합 제어
오차(ICE)는 어닐러의 제어 정밀도 한계다. 프로그래밍된 커플러는 대략 $0.02$--$0.03$
정도의 고정 오차를 갖는다. 정점에서 심은 커플러는 Ising 결합 $J\approx0.03$에 해당해
ICE와 비슷한 크기이고, 그래서 소자는 심은 신호를 일부만 분해한다. 이는 위에서 본 더
약한 정점과 부합한다. 스케일을 더 줄이면 커플러가 ICE 아래로 내려가 정점이
평탄해질 것이며, 이는 향후 과제로 남긴다.
