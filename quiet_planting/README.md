...
### N 스케일링 (alpha=4.2, field_strength=0.5, num_reads=50, num_sweeps=1000)

| N | QUBO 변수 | 성공률 | 평균 Hamming | 비고 |
|---:|:---------:|:------:|:----------:|------|
| 50 | ~260 | **100%** | 0.0 | SA-easy |
| 100 | ~520 | **60%** | 0.6 | 전이 시작 |
| 200 | ~1040 | **0%** | 4.6 | near-miss (Hamming 작음) |
| 500+ | ~2600+ | — | — | SA 비용 급증으로 미측정 |

**실패 양상**: Hamming distance가 2~5로, SA가 target 근처까지 접근하지만 마지막 몇 비트를 뒤집지 못하는 전형적 glassy 거동. Wishart(Hamming ≈ N/2)와 대조적.
...
Quiet Planting은 N=200에서 SA 전이가 발생하며, Wishart(N=100 전이)보다 약간 뒤에 위치한다. 전체 비교: [`docs/METHODOLOGY_COMPARISON.md`](../docs/METHODOLOGY_COMPARISON.md)
...
| `test_quiet_planted.py` | SA 실험 (alpha sweep, N scaling, 비교) |
