...
\begin{abstract}
Benchmarking QUBO solvers needs instances with a known optimum that stay hard.
Posiform planting builds such instances with one unique optimum. A recent
variant adds random spin-glass terms and scales the planted term by a
coefficient $\alpha$ to tune hardness. Hardness is usually graded by the
success rate. This rate is $0\%$ across the whole hard regime, so it cannot
rank instances, solvers, or $\alpha$. We instead measure how close the failing
runs come to the optimum. Over failures only, and on a fixed set of the hardest
instances, we record the energy gap and the Hamming distance (HD) to the
planted answer. Together they turn the flat $0\%$ into a continuous difficulty
curve. Under simulated annealing on a D-Wave Pegasus graph ($n{=}5612$), the
curve is non-monotone in $\alpha$. It rises to a peak, then declines. As
$\alpha$ grows, failing samples move closer in HD but deeper in energy. A single
supporting run on a D-Wave Advantage annealer shows the same shape on larger
gaps. This suggests the structure comes from the instances, not SA alone.
...
the hardness comes from ``local minima whose energy is very close to the true
ground state.''
...
A single supporting quantum-annealer run is consistent with this structure
...
($\Delta_R{=}1$). This unit is the same across instances, as is HD$/n$. At
$\alpha{=}0$, $x^\star$ is one of many $R$ ground states, so an energy match
($|\Delta E|<10^{-7}$) is not always a bit match. For $\alpha{>}0$ the ground
state is unique. We count a read as a success only on a bit match
($x{=}x^\star$). The $0\%$ success rate and the per-read failure rate both use
this criterion, not the ground-state energy, which is degenerate at $\alpha{=}0$.
...
and $2{\times}10^{-2}$, with extra points near the peak. Beyond $2{\times}10^{-2}$
...
reads per (instance,\,$\alpha$). For a cross-solver check we replay the same
...
negligible and $\Delta E$ stays flat at the bare $R$-trap depth. We call this
flat value the floor (mean $1.10$, median $1$). HD is also flat ($\approx73$). Past $\alpha{=}10^{-3}$ the planted term grows. $\Delta
...
even as their energy gap grows. The peak follows from
$\Delta E{=}\Delta R{+}\alpha\Delta P$, where $\Delta R$ and $\Delta P$ are
fixed for each bitstring. As failures near $x^\star$, their $\Delta P$ falls, so
$\Delta E$ peaks where rising $\alpha$ stops outpacing this fall. The peak is
...
of the instances, not of SA. At the peak the planted couplers map to Ising
$J\approx0.03$, near the device's integrated control error (ICE). We expect that
a smaller scale would push them below ICE and flatten the peak, which we leave
to future work.
...
success rate cannot. On a separate $150$-instance subset (not the hardest-$100$
set above), at $\alpha{=}10^{-3}$, $1000$ and $5000$ SA sweeps both give $0\%$
success, yet $\Delta E$ is $1.68$ versus $1.00$. The curve persists across SA
budgets and is echoed by the supporting QPU run. We therefore recommend
