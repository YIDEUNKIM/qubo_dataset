...
%==================== colours ====================
\definecolor{deepblue}{RGB}{29,90,150}    % title bar, block headers (#1D5A96)
...
\setbeamertemplate{headline}{
  ...
  % KNU logo on a white badge, flush-left and vertically centred in the title band
  \begin{tikzpicture}[remember picture, overlay]
    \node[anchor=west, fill=white, rounded corners=16pt, inner sep=10pt]
      at ([xshift=0.8cm, yshift=-8.2cm]current page.north west)
      {\includegraphics[height=11.5cm]{figures/knu_logo.png}};
  \end{tikzpicture}
}
...
\begin{itemize}
  \item \lab{Floor.} For $\alpha\in[10^{-9},10^{-4}]$, $\Delta E$ is flat at the depth of a local-minimum trap in $R$ --- mean $1.10$, median exactly $1$ (one $\Delta_R$); $\alpha\Delta P$ negligible. HD flat $\approx 73$.
...
\begin{block}{\bnum{6}$\Delta E$ ranks what the success rate cannot}
\RaggedRight
