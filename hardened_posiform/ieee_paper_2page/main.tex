...
empirically, through the solver success rate, which shows that difficulty varies
with the parameters but not why, and cannot compare instances once the rate hits
$0\%$. We instead measure how close failing runs come to the optimum. Over
failures only, on a fixed set of the hardest
instances, we record the energy gap and the Hamming distance (HD) to the
planted answer. These turn the flat $0\%$ into a continuous difficulty curve.
Under simulated annealing (SA) on a D-Wave Pegasus graph ($n{=}5612$), the curve
is non-monotone in $\alpha$: it rises to a peak, then declines, with failing
samples moving closer in HD but deeper in energy. A single supporting run on a
D-Wave Advantage annealer shows the same shape on larger gaps, suggesting the
structure comes from the instances, not SA alone.
...
success, yet $\Delta E$ is $1.68$ versus $1.00$. We therefore recommend
