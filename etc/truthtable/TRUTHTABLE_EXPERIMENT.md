...
> 설정: n=[3..14], 모드=greedy/approx(optimized), 10 runs, SA: reads=100, sweeps=5000
> 프리셋: preset_random_landscape
> 근사 모드: `create_qubo_approx_optimized` (A-free + SLSQP cutting plane)

| n | Greedy QUBO | Greedy Rate | Approx QUBO | Approx Rate | Greedy Gen | Approx Gen |
|:-:|:-----------:|:-----------:|:-----------:|:-----------:|:----------:|:----------:|
| 3 | 4 | **76.60%** | 3 | 64.90% | 0.000s | 0.021s |
| 4 | 6 | 38.20% | 4 | **70.10%** | 0.000s | 0.001s |
| 5 | 10 | 23.20% | 5 | **68.30%** | 0.000s | 0.002s |
| 6 | 18 | 7.50% | 6 | **51.20%** | 0.001s | 0.003s |
| 7 | 22 | 3.30% | 7 | **59.00%** | 0.002s | 0.538s |
| 8 | 30 | 1.20% | 8 | **53.40%** | 0.008s | 0.998s |
| 9 | 46 | 0.70% | 9 | **50.20%** | 0.026s | 2.327s |
| 10 | 78 | 0.20% | 10 | **59.10%** | 0.086s | 3.735s |
| 11 | 142 | 0.00% | 11 | **55.70%** | 0.484s | 5.581s |
| 12 | 270 | 0.00% | 12 | **54.30%** | 1.448s | 7.533s |
| 13 | 286 | 0.00% | 13 | **54.90%** | 5.097s | 9.454s |
| 14 | 318 | 0.00% | 14 | **54.70%** | **17.947s** | **12.466s** |
| 15 | - | - | 15 | **53.70%** | (>28분, 중단) | **16.057s** |
| 16 | - | - | 16 | **53.90%** | - | **18.662s** |

- **SA 교차점**: n=3에서만 greedy 우세, n≥4부터 approx가 역전
- **생성 속도 교차점**: n≤13에서는 greedy가 빠름, **n=14에서 approx가 역전** (17.9s vs 12.5s). Greedy의 aux 폭발(304개)이 지배적.
- **Greedy 실용 한계**: n≥15에서 greedy 생성 자체가 28분+ (1 run) → 실용성 없음
- **Approx 안정성**: n=3~16 전 범위에서 50~70% 유지 (n=16에서 53.9%, gen=18.7s)
- **Greedy 한계**: n=8부터 ~1% 이하, n≥11에서 0%
- **결론**: random landscape에서는 **n≥4부터 approx 모드 권장**. 생성 속도도 n≥14부터 approx가 유리. n≥15에서는 greedy 생성 자체가 비실용적.
...
|  | 정확 original | 정확 greedy | 근사 (QP optimized) |
|--|:------------:|:-----------:|:---------:|
| 보조변수 | O(n·2^n) 폭발 | **78~96% 절감** | **0개** |
| QUBO 크기 (n=8) | 530변수 | **30변수** | **8변수** |
| 에너지 정확도 | 100% 일치 | 100% 일치 | 근사 (RMSE ~0.7) |
| Ground state | 보장 | 보장 | **보장** (QP 제약) |
| SA 성공률 (n=5) | 10.70% | 23.20% | **68.30%** |
| SA 성공률 (n=16) | - | - | **53.90%** |
| 생성 속도 (n=16) | - | - | **18.7s** |
| 실용 한계 | n ≤ 4~5 | **n ≤ 13~14** | **n ≤ 23** |

---

### 실험 7: 근사 모드 Ground State 보장 검증 (Brute Force)

근사 모드(`create_qubo_approx_optimized`)가 다양한 seed에서 target을 유일한 ground state로 보장하는지 brute force 전수 검사.

> 설정: n=[5,7,8,10,12,14,16], 다양한 seed (n별 100~5개), per-seed 120s timeout
> 프리셋: preset_random_landscape
> 검증: 모든 2^n 상태 에너지 계산 → target이 유일한 최소인지 확인

| n | seeds | GS 성공 | Unique | avg gap | 시간 |
|:-:|:-----:|:-------:|:------:|:-------:|:----:|
| 5 | 100 | 100/100 | 100/100 | 0.021873 | 0.4s |
| 7 | 100 | 100/100 | 100/100 | 0.012747 | 33.3s |
| 8 | 50 | 50/50 | 50/50 | 0.010000 | 31.6s |
| 10 | 50 | 50/50 | 50/50 | 0.010000 | 209.9s |
| 12 | 30 | 30/30 | 30/30 | 0.010000 | 240.3s |
| 14 | 10 | 10/10 | 10/10 | 0.010000 | 136.4s |
| 16 | 5 | 5/5 | 5/5 | 0.010000 | 95.9s |

- **총 345 seeds, 전부 100% GS + Unique 보장**
- n>=8에서 avg gap이 정확히 0.01 (= epsilon)로 수렴 — SLSQP가 epsilon 제약을 tight하게 맞춤
- n=5,7에서는 lstsq가 자연스럽게 더 큰 마진 확보 (제약 없이도 GS 보장)
- timeout 발생 없음 (per-seed 120s 이내 생성 완료)
...
1. **보조변수 지수 폭발**: n=8 → greedy로도 30변수 (original: 530변수). SA 실용 한계 n ≤ 7~8. 생성 실용 한계 n ≤ 13~14 (n=15에서 1 run > 28분).
...
2. **SLSQP 생성 시간**: n=14에서 ~12.5초 (optimized). n≤23 실용적 (n=22 ~100s).
3. **진리표 크기 한계**: 입력이 2^n개이므로 n ≤ ~23에서만 진리표 열거 가능.
...
- **정확 모드 + greedy**: original 대비 보조변수 78~96% 절감. M 보정 후 n=7~8에서도 소폭 성공 (3.5~5.1%). n≥11에서 SA 성공률 0%.
- **근사 모드 (optimized)**: 보조변수 0개로 n≤23까지 실용적. Ground state 보장. n=3~14 전 범위에서 안정적 50~70%. 생성 속도도 n≥14부터 greedy 역전.
