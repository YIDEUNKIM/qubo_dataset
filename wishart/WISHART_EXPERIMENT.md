...
M = ⌊αN⌋ 개의 N차원 Gaussian 벡터 {w\_μ} (μ = 1, …, M)를 생성한 뒤, 각 벡터를 **t** 에 직교 투영한다:
...
대각 성분은 제거하고, 상삼각 부분만 J\_ij (i < j)로 추출한다.

**Step 4: Ground State 보장 증명**

Ising 에너지: H(s) = −Σ\_{i<j} J\_ij s\_i s\_j

**t** 에서의 에너지 기여:

```
−J_ij t_i t_j = (1/N)(WWᵀ)_ij t_i t_j
```

전체 에너지:

```
H(t) = (1/N) Σ_{i<j} (WWᵀ)_ij t_i t_j
     = (1/2N) tᵀ WWᵀ t  −  (대각 항)
```

**Wᵀt = 0** 이므로 **tᵀWWᵀt = ‖Wᵀt‖² = 0**.
...
s\_i = 2x\_i − 1 을 대입하여 Ising → QUBO:
...
**α** 가 제어하는 것은 **에너지 랜드스케이프의 복잡성**:

- **낮은 α (< 0.8)**: J 행렬이 low-rank → 에너지 랜드스케이프가 평탄하고 수많은 near-degenerate local minima 존재 → SA가 metastable 상태에 갇힘
- **높은 α (≥ 1.0)**: J 행렬이 full-rank에 가까움 → 명확한 global minimum → SA가 쉽게 탐색
- **상전이 구간 (α ≈ 0.8–0.95)**: 1차 상전이가 발생하며, 난이도가 급격히 변함
...
### 4.2 Alpha Sweep — 거친 탐색 (N=100, num\_reads=200, num\_sweeps=1000)
...
**관찰**: α ≈ 0.8 ~ 1.0에서 상전이 발생. α = 0.7에서 에너지 정확도가 최저(91.24%).
...
3. **Easy 구간 (alpha ≥ 0.95)**: SA가 100% ground state 도달. 임계점 α\_c ≈ 0.95.

4. **에너지 정확도 최저점**: alpha=0.75에서 92.20%로 최저. SA가 ground state 대비 **~8% 에너지 gap**에 갇힘.

### 4.4 num\_reads 영향 분석 (N=100, num\_reads=1 vs 200)

num\_reads=1 (단일 SA 실행)로 동일한 alpha sweep을 수행하여, SA sampling 횟수가 결과에 미치는 영향을 측정.
...
**전 구간 0% 성공.** num\_reads=200에서 100% 풀리던 alpha=0.95~1.0도 전부 실패.

#### num\_reads=200 vs num\_reads=1 비교
...
2. **Easy 구간 (alpha ≥ 0.95)**: reads 증가 효과 결정적 (14~16%). 개별 SA run의 성공 확률이 낮지만, 200번 반복하면 최소 1번은 ground state에 도달. num\_reads=1에서는 그 기회가 없어 전부 실패.
...
1. **1차 상전이**: 세밀한 sweep 결과, 임계점 α\_c ≈ 0.95 (N=100 기준). alpha=0.90에서 70%, alpha=0.95에서 100%로 급격히 전환. 성공률이 0% → 100%로 바뀌는 구간이 alpha 0.15 폭(0.80~0.95) 안에 집중됨.
...
|----------|-----------|---------|
| 난이도 제어 | noise\_ratio (간접) | alpha (직접, 이론적) |
| SA 실패 유도 | 불가능 (100% 성공) | 가능 (alpha<1.0에서 실패) |
| Ground state 보장 | 조건부 (W\_STRONG >> W\_WEAK) | 수학적 보장 (Wᵀt = 0) |
...
### 5.4 Zero-Expectation과의 구조적 비교 (num\_reads=1 벤치마크)

Zero-Expectation QUBO가 SA에게 trivially 쉬운 이유를 Wishart와 비교하여 분석.

#### num\_reads=1 Scaling 비교
...
1. **Frustration-Free vs Frustrated**: ZeroExp는 모든 pair가 독립적으로 타겟 방향을 가리켜 frustration이 없음. Wishart는 랜덤 프로젝션 W로 J\_ij를 구성하므로 pair 간 모순(frustration)이 발생하여 로컬 미니마를 다수 생성.
