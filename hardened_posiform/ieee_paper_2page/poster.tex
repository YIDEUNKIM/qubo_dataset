...
\RaggedRight
As \textbf{supporting evidence}, a \emph{single} run of the same $100$ instances on \textbf{D-Wave Advantage} gives $0\%$ success at every $\alpha$, yet the \textbf{same qualitative pattern}: a non-monotone $\Delta E$ peak near $\alpha{=}10^{-2}$ (weaker, $1.8\times$ vs $3.0\times$) and HD decreasing monotonically (\qpucol{$501\to167$}).
\begin{itemize}
  ...
  \item \lab{Precision floor.} Peak couplers map to Ising $J\approx 0.03$, near the scale where integrated control errors (ICE) matter; we \emph{hypothesize} that a smaller problem scale would push them below ICE and \textbf{flatten the peak} --- left to future verification.
\end{itemize}
\end{block}
...
\RaggedRight\large
Failure-conditioned \sacol{$\Delta E$} is the difficulty diagnostic and \qpucol{HD} its complement --- the all-sample mean (hence the success rate) hides the peak, falling at large $\alpha$ only because more reads succeed while the remaining failures stay deep. Whenever an $\alpha$-like hardness parameter is swept, we recommend reporting \textbf{failure-conditioned $\Delta E$ (floor \& peak) together with HD over a fixed set of hardest instances}. Because the curve persists across SA budgets and is echoed by a supporting quantum-annealer run, it provides a failure-conditioned \emph{diagnostic} of the benchmark instances rather than a single difficulty score, though absolute values remain solver- and schedule-dependent.
\end{block}
\end{column}
\begin{column}{\sepwid}\end{column}
\end{columns}
